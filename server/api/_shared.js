...
export function isSuperOrAdminRole(auth) {
  ...
  const configuredAdminEmail = (process.env.ADMIN_AUTH_EMAIL || process.env.VITE_ADMIN_AUTH_EMAIL || '').trim().toLowerCase();
  if (configuredAdminEmail && auth.user?.email && auth.user.email.toLowerCase() === configuredAdminEmail) return true;
  return false;
}
...
export async function verifyAdminAuth(req, sb) {
  ...
  try {
    ...
    const configuredAdminEmail = (process.env.ADMIN_AUTH_EMAIL || process.env.VITE_ADMIN_AUTH_EMAIL || '').trim().toLowerCase();
    const isSuperAdminEmail = Boolean(configuredAdminEmail) && user.email?.toLowerCase() === configuredAdminEmail;
