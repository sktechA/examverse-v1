...
function inferSubject(rawSubject,topic,questionText=''){
  ...
  if(/quantitative aptitude|mathematics|maths|quant|arithmetic/i.test(a) ||
     /simplification|percentage|percent|hcf|lcm|average|mean|number system|profit.*loss|compound interest|simple interest|ratio.*proportion|time.*work|time.*distance|speed|train|boat|algebra|geometry|trigonometry|mensuration|permutation|combination|probability|data interpretation/.test(t) ||
     /simplification|percentage|percent|hcf|lcm|average|mean|number system|profit.*loss|compound interest|simple interest|ratio.*proportion|time.*work|time.*distance|speed|train|boat|cost price|selling price|algebra|geometry|trigonometry|mensuration|permutation|combination|probability|data interpretation|प्रतिशत|लाभ और हानि|साधारण ब्याज|चक्रवृद्धि ब्याज|अनुपात|कार्य और समय|दूरी और समय|औसत|माध्य|महत्तम समापवर्तक|लघुत्तम समापवर्त्य|समीकरण|क्षेत्रमिति|प्रायिकता/.test(q)){
    return 'Mathematics';
  }
...
function numericOptionValues(r){
  return ['a','b','c','d'].map(k=>{
    const raw=String(r[`option_${k}`]||r[`option_${k}_hi`]||'').replace(/,/g,'');
    const m=raw.match(/-?\d+(?:\.\d+)?/);
    return m?Number(m[0]):null;
  });
}
function validateMathSemantics(r){
  const q=String(r.question||r.question_hi||'').toLowerCase();
  const opts=numericOptionValues(r);
  const errors=[];
  const hasNumericOptions=opts.filter(v=>v!==null).length>=2;
  const addIfExpectedMissing=(expected,label)=>{
    if(!hasNumericOptions){errors.push(`${label}: options are not numeric/relevant`);return;}
    const found=opts.some(v=>v!==null && Math.abs(v-expected)<1e-9);
    if(!found)errors.push(`${label}: expected value ${expected} is not present in options`);
  };
  if(/\baverage\b|औसत|माध्य/.test(q)){
    const nums=extractNumericValues(q);
    if(nums.length>=2) addIfExpectedMissing(nums.reduce((a,b)=>a+b,0)/nums.length,'Average validation');
  }
  if(/\bhcf\b|highest common factor|महत्तम समापवर्तक/.test(q)){
    const nums=extractNumericValues(q).filter(n=>Number.isInteger(n));
    if(nums.length>=2){let a=Math.abs(nums[0]),b=Math.abs(nums[1]);while(b){[a,b]=[b,a%b]} addIfExpectedMissing(a,'HCF validation');}
  }
  if(/\blcm\b|least common multiple|लघुत्तम समापवर्त्य/.test(q)){
    const nums=extractNumericValues(q).filter(n=>Number.isInteger(n));
    if(nums.length>=2){const gcd=(x,y)=>{x=Math.abs(x);y=Math.abs(y);while(y)[x,y]=[y,x%y];return x};const g=gcd(nums[0],nums[1]);const expected=Math.abs(nums[0]*nums[1])/(g||1);addIfExpectedMissing(expected,'LCM validation');}
  }
  const pct=q.match(/(-?\d+(?:\.\d+)?)\s*%[^\d]{0,20}(?:of|का|की|के)\s*(-?\d+(?:\.\d+)?)/);
  if(pct){addIfExpectedMissing(Number(pct[1])*Number(pct[2])/100,'Percentage validation');}
  const speed=q.match(/(-?\d+(?:\.\d+)?)\s*km[^\d]{0,20}(?:in|में)\s*(-?\d+(?:\.\d+)?)\s*(?:hours?|घंटे?)/);
  if(speed && /speed|गति|average speed|औसत गति/.test(q)){addIfExpectedMissing(Number(speed[1])/Number(speed[2]),'Speed validation');}
  return errors;
}
function validateQuestion(r){
  ...
  if(!r.subject) missing.push('Subject mapping unresolved');
  const mathErrors=validateMathSemantics(r);
  missing.push(...mathErrors);
...
function AdminExams({session}){
  ...
  const performAutoMap = async (examObj) => {
    ...
    try {
      ...
      // IMPORTANT: Never fill a subject section with another subject.
      // A shortage is safer than cross-subject contamination.
      if (selected.length < needed) {
        console.warn(`Question pool shortage for ${targetSubject}: ${needed - selected.length} questions not mapped.`);
      }
