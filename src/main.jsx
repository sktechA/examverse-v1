...
function bytesToLatin1(bytes){let out='';const chunk=0x8000;for(let i=0;i<bytes.length;i+=chunk){out+=String.fromCharCode(...bytes.subarray(i,Math.min(i+chunk,bytes.length)));}return out;}
async function inflateRaw(bytes){
  if(typeof DecompressionStream==='undefined')throw new Error('This browser cannot decompress DOCX/PDF streams. Please use TXT/CSV/XLSX.');
...
function decodePdfLiteral(raw){
  ...
  for(let i=0;i<raw.length;i++){
    const c=raw[i];
    if(c==='\\'&&i+1<raw.length){const n=raw[++i];const map={n:'\n',r:'\r',t:'\t',b:'\b',f:'\f'};if(map[n])out+=map[n];else if(/[0-7]/.test(n)){let oct=n;while(oct.length<3&&/[0-7]/.test(raw[i+1]||''))oct+=raw[++i];out+=String.fromCharCode(parseInt(oct,8));}else out+=n;}
    else out+=c;
  } return out;
}
function extractPdfTextFromStream(text){
  const out=[];let m;
  const re=/\[((?:\\.|[^\]])*)\]\s*TJ|\(((?:\\.|\\\)|\\\(|[^)])*)\)\s*Tj|<([0-9A-Fa-f\s]+)>\s*Tj/g;
  while((m=re.exec(text))){
    if(m[1]!==undefined){const parts=m[1].match(/\((?:\\.|\\\)|\\\(|[^)])*\)|<[^>]*>|-?\d+(?:\.\d+)?/g)||[];for(const p of parts){if(p[0]==='(')out.push(decodePdfLiteral(p.slice(1,-1)));else if(p[0]==='<'){const h=p.slice(1,-1).replace(/\s/g,'');for(let i=0;i<h.length;i+=2)out.push(String.fromCharCode(parseInt(h.slice(i,i+2),16)));}}out.push('\n');}
    else if(m[2]!==undefined)out.push(decodePdfLiteral(m[2]),'\n');
    else if(m[3]!==undefined){const h=m[3].replace(/\s/g,'');for(let i=0;i<h.length;i+=2)out.push(String.fromCharCode(parseInt(h.slice(i,i+2),16)));out.push('\n');}
  }
  return out.join('');
}
async function parsePdfQuestions(file){
  const bytes=new Uint8Array(await file.arrayBuffer());const raw=bytesToLatin1(bytes);if(!raw.startsWith('%PDF-'))throw new Error('Invalid PDF file.');
  let text='';let pos=0;
  while((pos=raw.indexOf('stream',pos))>=0){
    let start=pos+6;if(raw[start]==='\r')start++;if(raw[start]==='\n')start++;
    const end=raw.indexOf('endstream',start);if(end<0)break;
    const dictStart=Math.max(raw.lastIndexOf('<<',pos),0);const dict=raw.slice(dictStart,pos);let data=bytes.slice(start,end);
    while(data.length&&data[data.length-1]===10) data=data.slice(0,-1);while(data.length&&data[data.length-1]===13)data=data.slice(0,-1);
    try{if(/\/FlateDecode/.test(dict))data=await inflateRaw(data);const decoded=bytesToLatin1(data);if(/\b(?:BT|Tj|TJ|Tf|Td|Tm)\b/.test(decoded))text+='\n'+extractPdfTextFromStream(decoded);}catch(_){/* skip unsupported stream */}
    pos=end+9;
  }
  if(!text.trim())throw new Error('No extractable text found in this PDF. For scanned/image PDFs, use OCR or upload TXT/CSV/XLSX/DOCX.');
  return parseTxtQuestions(text,file.name);
}
async function parseDocxQuestions(file){
  const bytes=new Uint8Array(await file.arrayBuffer());if(bytes[0]!==0x50||bytes[1]!==0x4b)throw new Error('Invalid DOCX file.');
  let off=0,xml='';
  const view=new DataView(bytes.buffer,bytes.byteOffset,bytes.byteLength);
  while(off+30<bytes.length){
    if(view.getUint32(off,true)!==0x04034b50)break;
    const method=view.getUint16(off+8,true),compSize=view.getUint32(off+18,true),nameLen=view.getUint16(off+26,true),extraLen=view.getUint16(off+28,true);
    const name=bytesToLatin1(bytes.slice(off+30,off+30+nameLen));const dataStart=off+30+nameLen+extraLen;const data=bytes.slice(dataStart,dataStart+compSize);
    if(name==='word/document.xml'){
      const decoded=method===0?data:method===8?await inflateRaw(data):null;if(!decoded)throw new Error('Unsupported DOCX compression.');xml=bytesToLatin1(decoded);break;
    }
...
function Questions({session}){const inputRef=useRef(null),[file,setFile]=useState(null),[status,setStatus]=useState(''),[rows,setRows]=useState([]),[active,setActive]=useState('pending_review'),[loading,setLoading]=useState(false),[selected,setSelected]=useState([]),[stats,setStats]=useState({pending:0,approved:0,needs:0});
  const choose=e=>{const f=e.target.files?.[0];if(!f)return;const ext=f.name.split('.').pop().toLowerCase();const allowed=['txt','csv','xlsx','xls','pdf','docx'];if(!allowed.includes(ext)){setStatus('Unsupported format. Use TXT, CSV, XLSX, PDF or DOCX.');return}setFile(f);setStatus(`Selected ${f.name} — ready to process`)};
